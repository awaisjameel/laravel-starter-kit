...
final class UserCommandHandlerTest extends TestCase
{
    ...
    public function test_create_dispatches_management_event_with_context_metadata(): void
    {
        ...
        Event::assertDispatched(UserManagementEvent::class, static fn (UserManagementEvent $userManagementEvent): bool => $userManagementEvent->action === 'create'
            ...
            && $userManagementEvent->target?->is($userCommandResult->user) === true
            && $userManagementEvent->metadata->ipAddress === '127.0.0.1'
            && $userManagementEvent->metadata->userAgent === 'PHPUnit'
            && $userManagementEvent->metadata->socketId === '1234.5678'
    ...
    public function test_update_dispatches_management_event_with_audited_changes(): void
    {
        ...
        Event::assertDispatched(UserManagementEvent::class, static fn (UserManagementEvent $userManagementEvent): bool => $userManagementEvent->action === 'update'
            && $userManagementEvent->actor->is($actor)
            && $userManagementEvent->target?->is($target) === true
    ...
    public function test_delete_dispatches_management_event_before_removing_user(): void
    {
        ...
        Event::assertDispatched(UserManagementEvent::class, static fn (UserManagementEvent $userManagementEvent): bool => $userManagementEvent->action === 'delete'
            && $userManagementEvent->actor->is($actor)
            && $userManagementEvent->target?->id === $target->id
            && $userManagementEvent->metadata->socketId === '1234.5678'
            && $userManagementEvent->changes === []);
    }
    ...
    private function userActionContext(User $user): UserActionContext
    {
        return new UserActionContext(
            actor: $user,
            metadata: new UserActionMetadata(
                ipAddress: '127.0.0.1',
                userAgent: 'PHPUnit',
                socketId: '1234.5678',
